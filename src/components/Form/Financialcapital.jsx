...
function Financialcapital({ setCurrentPage, setMainFormData, mainFormData }) {
  ...
  // ตรวจสอบส่วน plants
  const validatePlants = (formData) => {
    const plants = formData.Agriculturalincome[0].plants;

    if (plants.length === 0) {
      Swal.fire({
        icon: "warning",
        title: "กรุณาเลือกอย่างน้อย 1 ข้อในส่วนรายได้จากการเกษตร",
        text: "โปรดเลือกตัวเลือกในส่วนรายได้จากการเกษตร",
        confirmButtonText: "ตกลง",
      });
      return false;
    }

    // ตรวจสอบเฉพาะ "ทำสวนผัก" และ "ทำสวนผลไม้"
    const hasInvalidPrefixPlant2 = plants.some((plant) =>
      plant.startsWith(Prefixplant2) && plant.slice(Prefixplant2.length + 1).trim() === ""
    );

    const hasInvalidPrefixPlant = plants.some((plant) =>
      plant.startsWith(Prefixplant) && plant.slice(Prefixplant.length + 1).trim() === ""
    );

    if (hasInvalidPrefixPlant2 || hasInvalidPrefixPlant) {
      Swal.fire({
        icon: "warning",
        title: "กรุณากรอกข้อมูลในช่องระบุของพืชที่เลือก",
        text: "โปรดกรอกข้อมูลในช่องระบุสำหรับตัวเลือก",
        confirmButtonText: "ตกลง",
      });
      return false;
    }
  ...
  // ตรวจสอบส่วน livestock
  const validateLivestock = (formData) => {
    const livestock = formData.Agriculturalincome[0].livestock;

    // ตรวจสอบว่ามีการเลือกอย่างน้อย 1 ตัวเลือกในส่วนปศุสัตว์
    if (livestock.length === 0) {
      Swal.fire({
        icon: "warning",
        title: "กรุณาเลือกอย่างน้อย 1 ข้อในส่วนปศุสัตว์",
        text: "โปรดเลือกตัวเลือกในส่วนปศุสัตว์",
        confirmButtonText: "ตกลง",
      });
      return false;
    }

    // ตรวจสอบกรณี "อื่นๆ (กบ/ปู/ปลิง/ผึ้ง)"
    const otherLivestock = livestock.find((item) =>
      item.startsWith(Prefixlivestock)
    );
    if (otherLivestock && otherLivestock.slice(Prefixlivestock.length + 1).trim() === "") {
      Swal.fire({
        icon: "warning",
        title: "กรุณาระบุข้อมูล'",
        text: "โปรดกรอกข้อมูลในช่องระบุสำหรับตัวเลือก อื่นๆ",
        confirmButtonText: "ตกลง",
      });
      return false;
    }
  ...
  // ตรวจสอบส่วน fishing
  const validateFishing = (formData) => {
    const fishing = formData.Agriculturalincome[0].fishing;

    if (fishing.length === 0) {
      Swal.fire({
        icon: "warning",
        title: "กรุณาเลือกอย่างน้อย 1 ข้อในส่วนประมงค์",
        text: "โปรดเลือกตัวเลือกในส่วนประมงค์",
        confirmButtonText: "ตกลง",
      });
      return false;
    }
  ...
  const validateNonAGIincome = () => {
    if (formData.NonAGIincome.length === 0) {
      Swal.fire({
        icon: "warning",
        title: "กรุณาเลือกอย่างน้อย 1 รายได้",
        text: "โปรดเลือกตัวเลือกในส่วนรายได้ที่นอกภาคการเกษตร (เฉลี่ย/ปี)",
        confirmButtonText: "ตกลง",
      });
      return false; // Validation failed
    }

    const incomeTypesWithoutCost = [prefixIncomeType2];
    const incompleteEntries = formData.NonAGIincome.some((item) => {
      const isAmountMissing = !item.amount_per_year || isNaN(item.amount_per_year);
      const isCostMissing =
        !incomeTypesWithoutCost.includes(item.income_type) &&
        (!item.cost_per_year || isNaN(item.cost_per_year));
      return isAmountMissing || isCostMissing;
    });
    ...
    if (incompleteEntries) {
      Swal.fire({
        ...
        text: "โปรดระบุรายได้และต้นทุน (ถ้าจำเป็น) ของแต่ละรายการที่เลือกให้ครบถ้วน",
        confirmButtonText: "ตกลง",
      });
      return false; // Validation failed
    }
  ...
  const validateHouseholdExpenses = (formData) => {
    const { Householdexpenses } = formData;

    // ตรวจสอบว่ามีอย่างน้อย 1 รายการที่ถูกเลือกและมีค่าใช้จ่าย
    const hasSelected = Householdexpenses.some((item) => item.amount_per_month > 0);

    if (!hasSelected) {
      Swal.fire({
        icon: "warning",
        title: "กรุณาเลือกอย่างน้อย 1 ข้อ",
        text: "โปรดระบุประเภทค่าใช้จ่ายและจำนวนเงิน",
        confirmButtonText: "ตกลง",
      });
      return false; // ไม่ผ่านการตรวจสอบ
    }

    // ตรวจสอบว่ามีค่าใช้จ่ายที่เลือก แต่ไม่ได้กรอกจำนวนเงิน
    const incompleteExpenses = Householdexpenses.filter(
      (item) => item.expenses_type && (!item.amount_per_month || item.amount_per_month <= 0)
    );
    ...
    if (incompleteExpenses.length > 0) {
      Swal.fire({
        icon: "warning",
        title: "กรุณากรอกข้อมูลให้ครบ",
        text: "โปรดกรอกจำนวนเงินในทุกประเภทค่าใช้จ่ายที่เลือก",
        confirmButtonText: "ตกลง",
      });
      return false; // ไม่ผ่านการตรวจสอบ
    }

    // ตรวจสอบกรณี "ค่าใช้จ่ายอื่น ๆ" (ข้อ 11)
    const otherExpense = Householdexpenses.find((item) =>
      item.expenses_type.startsWith(prefixExperss11)
    );

    if (otherExpense) {
      if (!otherExpense.expenses_type || otherExpense.expenses_type === prefixExperss11 || !otherExpense.amount_per_month) {
        Swal.fire({
          icon: "warning",
          title: "กรุณากรอกข้อมูลให้ครบ",
          text: "สำหรับ 'ค่าใช้จ่ายอื่น ๆ' โปรดระบุประเภทค่าใช้จ่าย",
          confirmButtonText: "ตกลง",
        });
        return false; // ไม่ผ่านการตรวจสอบ
      }
  ...
  const validateSavings = (formData) => {
    const { Saving } = formData;

    // ตรวจสอบว่าเลือกสถานะการออมหรือยัง
    const hasSavingSelected = Saving[0]?.is_has_saving !== null && Saving.length > 0;

    if (!hasSavingSelected) {
      Swal.fire({
        icon: "warning",
        title: "กรุณาเลือกสถานะการออม",
        text: "โปรดเลือกว่าครัวเรือนของท่านมีการออมหรือไม่",
        confirmButtonText: "ตกลง",
      });
      return false; // ไม่ผ่านการตรวจสอบ
    }

    // ตรวจสอบเมื่อเลือก "มีการออม"
    if (Saving[0]?.is_has_saving === true) {
      // ตรวจสอบว่ามีประเภทการออมและจำนวนเงินในทุกตัวเลือกที่ถูกเลือก
      const incompleteSavings = Saving.filter(
        (item) =>
          item.saving_type.trim() !== "" && // ประเภทการออมถูกระบุ
          (item.amount === null || item.amount <= 0) // จำนวนเงินไม่ครบหรือไม่ถูกต้อง
      );

      if (incompleteSavings.length > 0) {
        Swal.fire({
          icon: "warning",
          title: "กรุณากรอกข้อมูลการออมให้ครบ",
          text: "โปรดระบุประเภทการออมและยอดเงินรวมในทุกตัวเลือกที่เลือก",
          confirmButtonText: "ตกลง",
        });
        return false; // ไม่ผ่านการตรวจสอบ
      }

      // ตรวจสอบกรณี "การออมอื่นๆ"
      const otherSaving = Saving.find((item) =>
        item.saving_type.startsWith(prefixSaving)
      );

      if (otherSaving) {
        if (
          otherSaving.saving_type.trim() === prefixSaving.trim() || // ไม่ได้ระบุรายละเอียดประเภท
          otherSaving.amount <= 0 // ไม่ได้ระบุจำนวนเงิน
        ) {
          Swal.fire({
            icon: "warning",
            title: "กรุณากรอกข้อมูลการออมอื่นๆ ให้ครบ",
            text: "สำหรับ 'การออมอื่นๆ' โปรดระบุประเภทการออมและยอดเงินรวม",
            confirmButtonText: "ตกลง",
          });
          return false; // ไม่ผ่านการตรวจสอบ
        }
  ...
  const validateOccupationalProperty = (formData) => {
    // ตรวจสอบว่าได้เลือกสถานะ "มีทรัพย์สิน" หรือไม่
    const hasPropertySelected = formData.Occupationalproperty[0]?.is_has_property !== null;

    if (!hasPropertySelected) {
      Swal.fire({
        icon: "warning",
        title: "กรุณาเลือกสถานะทรัพย์สินเพื่อประกอบอาชีพ",
        text: "โปรดเลือกว่าครัวเรือนของท่านมีทรัพย์สินเพื่อประกอบอาชีพหรือไม่",
        confirmButtonText: "ตกลง",
      });
      return false; // ไม่ผ่านการตรวจสอบ
    }

    // หากเลือกว่า "มีทรัพย์สิน" ต้องเลือกประเภททรัพย์สิน
    if (
      formData.Occupationalproperty[0]?.is_has_property === true &&
      formData.Occupationalproperty[0]?.property_type.length === 0
    ) {
      Swal.fire({
        icon: "warning",
        title: "กรุณาเลือกประเภททรัพย์สิน",
        text: "โปรดเลือกประเภททรัพย์สินที่ใช้ประกอบอาชีพ",
        confirmButtonText: "ตกลง",
      });
      return false; // ไม่ผ่านการตรวจสอบ
    }

    // ตรวจสอบกรณี "อื่น ๆ (ระบุ)" หากเลือก "อื่น ๆ" จะต้องกรอกข้อมูลในช่อง input text
    const otherProperty = formData.Occupationalproperty[0]?.property_type.find((property) =>
      property.startsWith(prefix)
    );
    if (otherProperty && otherProperty.slice(prefix.length + 1).trim() === "") {
      Swal.fire({
        icon: "warning",
        title: "กรุณากรอกข้อมูลใน 'อื่น ๆ'",
        text: "โปรดระบุรายละเอียดเพิ่มเติมสำหรับทรัพย์สินประเภท 'อื่น ๆ'",
        confirmButtonText: "ตกลง",
      });
      return false; // ไม่ผ่านการตรวจสอบ
    }
  ...
  const validateDebt = (formData) => {
    const debt = formData.Debt[0];

    // ตรวจสอบว่าได้เลือกสถานะหนี้สินหรือไม่
    if (debt.is_has_debt === null) {
      Swal.fire({
        icon: "warning",
        title: "กรุณาเลือกสถานะหนี้สิน",
        text: "โปรดเลือกว่าครัวเรือนของท่านมีหนี้สินหรือไม่",
        confirmButtonText: "ตกลง",
      });
      return false; // ไม่ผ่านการตรวจสอบ
    }

    // กรณีเลือก "ไม่มีหนี้สิน" (is_has_debt === false)
    if (debt.is_has_debt === false) {
      // ตรวจสอบว่ามีการกรอกข้อมูล description หรือไม่
      if (!debt.description || debt.description.trim() === "") {
        Swal.fire({
          icon: "warning",
          title: "กรุณาระบุเหตุผลสำหรับสถานะ 'ไม่มีหนี้สิน'",
          text: "โปรดเลือกเหตุผลอย่างน้อยหนึ่งข้อ",
          confirmButtonText: "ตกลง",
        });
        return false; // ไม่ผ่านการตรวจสอบ
      }

      // ตรวจสอบกรณี "อื่น ๆ (ระบุ)"
      if (debt.description.startsWith(prefix) && !debt.description.includes(": ")) {
        Swal.fire({
          icon: "warning",
          title: "กรุณากรอกข้อมูลใน 'อื่น ๆ'",
          text: "โปรดระบุรายละเอียดเพิ่มเติมในช่อง 'อื่น ๆ'",
          confirmButtonText: "ตกลง",
        });
        return false; // ไม่ผ่านการตรวจสอบ
      }
    ...
    // กรณีเลือก "มีหนี้สิน" (is_has_debt === true)
    if (debt.is_has_debt === true) {
      // ตรวจสอบว่า Credit sources ได้ถูกเลือกหรือไม่
      if (formData.Creditsources.length === 0) {
        Swal.fire({
          icon: "warning",
          title: "กรุณาเลือกแหล่งเงินกู้",
          text: "โปรดเลือกแหล่งเงินกู้ที่เกี่ยวข้อง",
          confirmButtonText: "ตกลง",
        });
        return false; // ไม่ผ่านการตรวจสอบ
      }

      // ตรวจสอบว่าแต่ละ Credit source มี outstanding_amount กรอกหรือไม่
      const incompleteCredits = formData.Creditsources.some(
        (source) => !source.outstanding_amount || source.outstanding_amount <= 0
      );
      if (incompleteCredits) {
        Swal.fire({
          icon: "warning",
          title: "กรุณากรอกจำนวนเงินคงค้าง",
          text: "โปรดกรอกจำนวนเงินคงค้างของแหล่งเงินกู้ทั้งหมด",
          confirmButtonText: "ตกลง",
        });
        return false; // ไม่ผ่านการตรวจสอบ
      }

      // ตรวจสอบกรณี "อื่น ๆ (ระบุ)" สำหรับ prefixCredit2
      const otherCredit2 = formData.Creditsources.find((source) =>
        source.form.startsWith(prefixCredit2)
      );
      if (otherCredit2 && otherCredit2.form.slice(prefixCredit2.length + 1).trim() === "") {
        Swal.fire({
          icon: "warning",
          title: "กรุณากรอกข้อมูลใน 'อื่น ๆ'",
          text: "โปรดระบุรายละเอียดเพิ่มเติมสำหรับแหล่งเงินกู้ 'อื่น ๆ (4)'",
          confirmButtonText: "ตกลง",
        });
        return false; // ไม่ผ่านการตรวจสอบ
      }

      // ตรวจสอบกรณี "อื่น ๆ (ระบุ)" สำหรับ prefixCredit
      const otherCredit = formData.Creditsources.find((source) =>
        source.form.startsWith(prefixCredit)
      );
      if (otherCredit && otherCredit.form.slice(prefixCredit.length + 1).trim() === "") {
        Swal.fire({
          icon: "warning",
          title: "กรุณากรอกข้อมูลใน 'อื่น ๆ'",
          text: "โปรดระบุรายละเอียดเพิ่มเติมสำหรับแหล่งเงินกู้ 'อื่น ๆ (12)'",
          confirmButtonText: "ตกลง",
        });
        return false; // ไม่ผ่านการตรวจสอบ
      }
  ...
  const validateAllSections = (formData) => {
    // ตรวจสอบแต่ละส่วน
    return (
      validatePlants(formData) && // ตรวจสอบ plants
      validateLivestock(formData) && // ตรวจสอบ livestock
      validateFishing(formData) && // ตรวจสอบ fishing
      validateNonAGIincome() && // ตรวจสอบ NonAGIincome
      validateHouseholdExpenses(formData) && // ตรวจสอบ HouseholdExpenses
      validateSavings(formData) && // ตรวจสอบ Savings
      validateDebt(formData) && // ตรวจสอบ Debt
      validateOccupationalProperty(formData) // ตรวจสอบ OccupationalProperty
    );
  };
