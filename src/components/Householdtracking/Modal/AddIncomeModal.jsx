...
const AddIncomeModal = ({ show, onClose, household, reloadData }) => {
  ...
  const [formAGIincome, setFromAGIincome] = useState({
    type: "",
  ...
  const handleInputChange = (field, value) => {
    const updateData = { ...formAGIincome };
    updateData[field] = value;
    setFromAGIincome(updateData);
  };

  //checkbox AGI financial
  const handleCheckbocAGIfinancial = (checked) => {
    const updateData = { ...formAGIincome };
    if (checked) {
      updateData.type = "รายได้จากการเกษตร";
    } else {
      updateData.type = "";
    }
    setFromAGIincome(updateData);
  ...
  const validateFormData = () => {
    ...
    formData.NonAGIincome.forEach((item) => {
      ...
      if (
        item.cost_per_year === null ||
        item.cost_per_year === undefined ||
        isNaN(item.cost_per_year)
      ) {
        Swal.fire(
          "ข้อผิดพลาด",
    ...
    //ตรวจสอบรายได้จากการเกษตร
    if (
      formAGIincome.type &&
      (formAGIincome.amount_per_year == null ||
        formAGIincome.amount_per_year == undefined)
    ) {
      Swal.fire({
        title: "ข้อผิดพลาด",
        text: "กรณากรอกข้อมูลรายได้จากการเกษตรให้ครบถ้วน",
        icon: "error",
      });
      isValid = false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const hasNonAGIData = formData.NonAGIincome.length > 0;
    const hasAGIData =
      formAGIincome.type && formAGIincome.amount_per_year != null;

    if (!hasNonAGIData && !hasAGIData) {
      await Swal.fire({
        title: "ข้อผิดพลาด",
    ...
    if (hasNonAGIData && !validateFormData()) {
      return;
    }
    ...
    try {
      ...
      if (res.isConfirmed) {
        ...
        // Add Non-AGI income API call if data exists
        if (hasNonAGIData) {
          apiCalls.push(
            axios.post(
              `${config.api_path}/house-hold/create-nonagiincome/${household.id}`,
              formData.NonAGIincome,
              config.headers()
            )
          );
        }

        // Add AGI income API call if data exists
        if (hasAGIData) {
          apiCalls.push(
            axios.post(
              `${config.api_path}/house-hold/create-agi-financial/${household.id}`,
              formAGIincome,
              config.headers()
            )
          );
        ...
        const allSuccessful = responses.every(
          (response) =>
            response.data.message ===
              "NonAGIincome records created successfully" ||
            response.data.message ===
              "success"
        );
  ...
  return (
    <>
      ...
      >
        ...
        <div className="mb-6">
          ...
          <div className="pl-10">
            ...
            <div className="flex flex-wrap items-center mb-4">
              ...
              />
              <label htmlFor="income_type_0" className="font text-black mr-2">
              ...
              {formData.NonAGIincome.some(
                ...
                <div className="mt-3 flex flex-wrap items-center sm:w-auto md:w-2/2 lg:w-auto xl:w-auto ">
                  <input
                    type="number"
                    placeholder="....บาท/ปี"
            ...
            <div className="flex items-center mb-4">
              ...
              />
              <label htmlFor="income_type_1" className="font text-black mr-2">
        ...
        <div className="mb-6">
          <h3 className="text-lg font-bold text-black mb-5 ">
            รายได้จากภาคการเกษตร (เฉลี่ย/ปี)
          </h3>
          <div className="pl-10">
            <div className="flex flex-wrap items-center mb-4">
              <input
                ...
                name="income_type"
                className="mr-2 rounded"
                checked={formAGIincome.type === "รายได้จากการเกษตร"}
                onChange={(e) => handleCheckbocAGIfinancial(e.target.checked)}
              />
              <label htmlFor="income_type_0" className="font text-black mr-2">
                มีรายได้จากการเกษตร
              </label>

              {formAGIincome.type === "รายได้จากการเกษตร" && (
                <div className="flex flex-wrap items-center sm:w-auto md:w-2/2 lg:w-auto xl:w-auto ">
                  <input
                    ...
                    className="border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-blue-400 shadow-md transition duration-200  sm:w-48 md:w-48 lg:w-60 xl:w-60"
                    value={formAGIincome.amount_per_year}
                    onChange={(e) =>
